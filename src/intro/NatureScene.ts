...
export class NatureScene {
  ...
  private buildTile() {
    const floorMat = new THREE.MeshStandardMaterial({ color: "#d6d6cb", roughness: .96 });
    floorMat.onBeforeCompile = shader => {
      ...
      shader.fragmentShader = "varying vec3 vGround; uniform float uGardenTime;\n" + shader.fragmentShader;
      shader.fragmentShader = shader.fragmentShader.replace("#include <color_fragment>", `
        #include <color_fragment>
        float canopy=sin(vGround.x*2.7+sin(vGround.z*1.6)+uGardenTime*.05)*cos(vGround.z*3.2-vGround.x*.7);
        diffuseColor.rgb*=1.-smoothstep(.1,.8,canopy)*.16;
      `);
    };
    ...
    material.onBeforeCompile = shader => {
      ...
      shader.vertexShader = shader.vertexShader.replace("#include <begin_vertex>", "#include <begin_vertex>\nvTile=position;");
      shader.fragmentShader = "varying vec3 vTile; uniform float uGarden; uniform float uGardenTime;\n" + shader.fragmentShader;
      shader.fragmentShader = shader.fragmentShader.replace("#include <color_fragment>", `
        #include <color_fragment>
        ...
        diffuseColor.rgb*=1.-smoothstep(.1,.8,canopy)*.24;
      `);
    };
