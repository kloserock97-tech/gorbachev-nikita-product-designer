...
/* Две истории подряд, как в igloo: сначала холм «выгружается» — камера отъезжает, сетка
   голограммы съедает луг от краёв к креслу, нарастает глитч, снизу поднимается заливка;
   под заливкой — светлая страница About, и на неё прилетает компьютер. */
export const STORY = {
  ...
  holoIn: [0.08, 0.14] as const, // голограмма проявляется
  reveal: [0.1, 0.36] as const, // сетка съедает луг: радиус «живой» зоны 24 м → 0
  walls: [0.12, 0.32] as const, // стены-экраны растут
  net: [0.2, 0.34] as const, // квадратная сетка → треугольная
  glitch: [0.22, 0.36, 0.45] as const, // вход, пик, выход глитча
  fill: [0.24, 0.46] as const, // заливка снизу закрывает кадр
  overlay: 0.1, // с этого места компьютер рисуется своим слоем — поверх сетки, глитча и заливки
