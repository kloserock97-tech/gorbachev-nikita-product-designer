...
export class HillScene {
  ...
  private updateStory(dt: number) {
    ...
    fx.alt = alt;
    if (alt > 0) {
      /* тёмный грейд Meadow Walk: приглушённый цвет, контраст, тяжёлая виньетка (поверх погоды холма) */
      fx.vibrance += (-0.5 - fx.vibrance) * alt;
      fx.contrast += (1.12 - fx.contrast) * alt;
      fx.vignette = this.vignetteBase + (0.85 - this.vignetteBase) * alt;
